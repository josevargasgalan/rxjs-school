...
export default () => {
  ...
  const getComments = () => {
    ...
    return concat(comment1$, comment2$, comment3$, comment4$)
      .pipe(
        map(JSON.stringify),
        endWith('--------//--------')
      )
  ...
  /** MergeAll - se usa en observables que emiten observables (High Order Observables),
   * se suscribe a los observables internos y emite sus eventos como si fueran eventos
   * del HOO*/
  ...
  /** MergeMap - MergeMap se suscribe a los observables internos y emite sus valores a 
   * través del observable externo o HOO
   */
  fromEvent(button, 'click')
  .pipe(
